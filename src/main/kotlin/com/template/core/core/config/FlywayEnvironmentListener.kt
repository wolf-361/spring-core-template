...
class FlywayEnvironmentListener : ApplicationListener<ApplicationEnvironmentPreparedEvent> {
    ...
    override fun onApplicationEvent(event: ApplicationEnvironmentPreparedEvent) {
        ...
        val schemas = env.getProperty("spring.flyway.schemas")
        val defaultSchema = env.getProperty("spring.flyway.default-schema")
        ...
        try {
            val flyway =
                Flyway
                    ...
                    .apply {
                        if (!schemas.isNullOrBlank()) schemas(*schemas.split(",").map { it.trim() }.toTypedArray())
                        if (!defaultSchema.isNullOrBlank()) defaultSchema(defaultSchema)
                    }.load()
