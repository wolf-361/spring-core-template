...
1. **Pure domain + separate persistence model** — `domain/Item` is a plain Kotlin data class. A separate `infrastructure/persistence/ItemJpaEntity` holds the JPA annotations. A mapper converts between them.
2. **Annotated domain entity** — `domain/Item` carries `@Entity`, `@Column`, `@Id` directly. The same class is stored by JPA and used throughout the application.
...
The domain model IS the persistence model. There is one `Item` class (per feature), annotated with JPA, located in `features/<name>/domain/model/`.
...
- **Changes propagate cleanly.** Adding a field to `Item` is one change, not three synchronized changes across domain, persistence entity, and mapper.
- **Readable code.** A new contributor sees `Item` and understands both the business concept and how it's stored. No jumping between four files.
...
Rejected as over-engineering for this template.
